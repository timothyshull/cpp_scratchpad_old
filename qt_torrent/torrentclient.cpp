...
void TorrentClient::setPaused(bool paused) {
    if (paused) {
        ...
        // resume later.
        d->setState(Paused);
                foreach (PeerWireClient *client, d->connections)client->abort();
        d->connections.clear();
        TorrentServer::instance()->removeClient(this);
    } else {
...
void TorrentClient::scheduleDownloads() {
    ...
    // who are either idle or done.
            foreach (PeerWireClient *client, d->connections)schedulePieceForClient(client);
}
...
void TorrentClient::schedulePieceForClient(PeerWireClient *client) {
    ...
    if (!piece) {
        ...
        // Remove all pieces that this client has already requested.
                foreach (int i, currentPieces)incompletePiecesAvailableToClient.clearBit(i);
